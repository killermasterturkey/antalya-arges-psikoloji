              ...
              {therapyAreas.map((area, index) => (
                ...
                >
                  <div className="w-16 h-16 bg-olive/20 rounded-2xl flex items-center justify-center mx-auto">
                    <i className={`${area.icon} text-3xl text-olive`}></i>
                  </div>
                  <h3 className="font-sans text-lg font-bold text-darkgray">{area.title}</h3>
                  <p className="font-sans text-sm text-darkgray/70">{area.description}</p>
                </motion.div>
              ))}
            </div>
          </div>
...
export default AntalyaCocukVeOyunPage;
