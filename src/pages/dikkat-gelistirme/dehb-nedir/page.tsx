...
const DehbNedirPage = () => {
  const symptoms = [
    { icon: 'ri-focus-3-line', title: 'Dikkat Eksikliği', description: 'Odaklanma güçlüğü, detayları kaçırma, dinlememe.' },
    { icon: 'ri-run-line', title: 'Hiperaktivite', description: 'Aşırı hareketlilik, yerinde duramama, sürekli konuşma.' },
    { icon: 'ri-flashlight-line', title: 'Dürtüsellik', description: 'Düşünmeden hareket etme, sıra bekleyememe.' },
    { icon: 'ri-time-line', title: 'Zaman Yönetimi', description: 'Görevleri tamamlayamama, erteleme eğilimleri.' },
  ];
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              ...
              >
                <div className="inline-flex items-center space-x-2 bg-white border border-darkgray/20 px-4 py-2 rounded-full">
                  <i className="ri-question-line text-olive"></i>
                  <span className="font-sans text-sm text-darkgray">Dikkat Geliştirme</span>
                </div>
                ...
                <div className="flex flex-wrap gap-4 pt-4">
                  <Link to="/cocuklar-icin-moxo" className="inline-flex items-center space-x-2 bg-darkgray text-white px-8 py-4 rounded-full font-sans text-base font-medium hover:bg-olive transition-all duration-300 shadow-md hover:shadow-lg cursor-pointer whitespace-nowrap">
                    <i className="ri-bear-smile-line"></i>
                    <span>Çocuklar İçin MOXO</span>
                  </Link>
                  <Link to="/yetiskinler-icin-moxo" className="inline-flex items-center space-x-2 bg-white text-darkgray px-8 py-4 rounded-full font-sans text-base font-medium hover:bg-darkgray hover:text-white transition-all duration-300 border border-darkgray/20 cursor-pointer whitespace-nowrap">
                    <i className="ri-user-line"></i>
                    <span>Yetişkinler İçin MOXO</span>
                  </Link>
                </div>
              </motion.div>
              ...
              {symptoms.map((symptom, index) => (
                ...
                >
                  <div className="w-16 h-16 bg-olive/20 rounded-2xl flex items-center justify-center mx-auto">
                    <i className={`${symptom.icon} text-3xl text-olive`}></i>
                  </div>
                  <h3 className="font-sans text-lg font-bold text-darkgray">{symptom.title}</h3>
                  <p className="font-sans text-sm text-darkgray/70">{symptom.description}</p>
                </motion.div>
              ))}
            </div>
          </div>
